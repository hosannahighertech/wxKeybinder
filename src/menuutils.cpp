...
{
	wxString str = m_pItem->GetItemLabel();
	...
	str.Trim();
	if (str == m_pItem->GetItemLabel())
		str += wxT(" ");
#endif

	if (m_nShortcuts <= 0) {
	
		wxKBLogDebug(wxT("wxMenuCmd::Update - no shortcuts defined for [%s]"), str.c_str().AsChar());
	...
	wxKBLogDebug(wxT("wxMenuCmd::Update - setting the new text to [%s]"), newtext.c_str().AsChar());
...
void wxMenuWalker::WalkMenuItem(wxMenuBar *p, wxMenuItem *m, void *data)
{
	wxKBLogDebug(wxT("wxMenuWalker::WalkMenuItem - walking on [%s] at level [%d]"), 
				m->GetItemLabel().c_str().AsChar(), m_nLevel);
	...
	if (m->GetSubMenu()) {
		...
		wxKBLogDebug(wxT("wxMenuWalker::WalkMenuItem - recursing on [%s]"), m->GetItemLabel().c_str().AsChar());
		m_nLevel++;
...
void wxMenuWalker::WalkMenu(wxMenuBar *p, wxMenu *m, void *data)
{
	wxKBLogDebug(wxT("wxMenuWalker::WalkMenu - walking on [%s] at level [%d]"), 
				m->GetTitle().c_str().AsChar(), m_nLevel);
	for (int i=0; i < (int)m->GetMenuItemCount(); i++) {
		...
		if (pitem->GetKind() != wxITEM_SEPARATOR && 
			pitem->GetItemLabel() != wxEmptyString)
...
void wxMenuWalker::Walk(wxMenuBar *p, void *data)
{
	...
	for (int i=0; i < (int)p->GetMenuCount(); i++) {
		...
		wxKBLogDebug(wxT("wxMenuWalker::Walk - walking on [%s] at level [%d]"), 
					p->GetMenuLabel(i).c_str().AsChar(), m_nLevel);
		void *tmp = OnMenuWalk(p, m, data);
...
{
	wxKBLogDebug(wxT("wxMenuWalker::OnMenuWalk - walking on [%s]"), m->GetTitle().c_str().AsChar());
	wxString toadd;
	...
	wxKBLogDebug(wxT("wxMenuWalker::OnMenuWalk - appending [%s]"), toadd.c_str().AsChar());
...
{
	wxKBLogDebug(wxT("wxMenuWalker::OnMenuItemWalk - walking on [%s]"), m->GetItemLabel().c_str().AsChar());
	//int last = m_pCategories->GetCount()-1;
	wxExComboItemData *p = (wxExComboItemData *)data;//m_pCategories->GetClientObject(last);

	// append a new item
	if (m->GetSubMenu() == NULL)
		p->Append(m->GetItemLabel(), m->GetId());
	else
		m_strAcc += wxT(" | ") + m->GetItemLabel();
...
{
	wxKBLogDebug(wxT("wxMenuWalker::OnMenuExit - walking on [%s]"), m->GetTitle().c_str().AsChar());
...
void *wxMenuShortcutWalker::OnMenuItemWalk(wxMenuBar *, wxMenuItem *m, void *)
{
	...
	wxCmd *cmd = new wxMenuCmd(m, m->GetItemLabel(), m->GetHelp());
	m_pArr->Add(cmd);
